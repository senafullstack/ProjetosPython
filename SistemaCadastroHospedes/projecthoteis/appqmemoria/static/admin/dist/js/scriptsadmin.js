...
/* EXCLUIR SELECIONADOS*/ 
$('#modal_excluir_todos').on('show.bs.modal', function (e) {
  $message = $(e.relatedTarget).attr('data-message');
  $(this).find('.modal-body p').text($message);
  $title = $(e.relatedTarget).attr('data-title');
  $(this).find('.modal-title').text($title);
...
$('#modal_excluir_todos').find('.modal-footer #confirm').on('click', function () {
  $("#form_excluirtodos").submit();
});
...
function readURL(input, foto) {
  if (input.files && input.files[0]) {
    ...
    reader.onload = function (e) {
      var result = ".img_upload_" + foto;
      $(result).attr('src', e.target.result);
    }
    reader.readAsDataURL(input.files[0]);
  }
}
function carregaFoto(input, foto) {
  //alert(foto);
  readURL(input, foto);
}
$(function () {
